...
def create_prompt_formats(example,tokenizer):
    ...
    user_variations = [
        ...
        "Task: Provide all the steps of Gcode for the following milling problem",
        "Command: Provide a detailed Gcode to tackle the milling operation given below"
    ]
    ...
    messages = [
        {"role": "system", "content": "You are expert in Generating Gcode for HaaS mill"},
...
def preprocess_dataset(tokenizer: AutoTokenizer, max_length: int,seed, dataset):
    ...
    dataset = dataset.map(
        ...
        batched=True,
        remove_columns=['index','prompt', 'response'],
        num_proc=1
    )
